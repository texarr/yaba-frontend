...
export class CategoriesComponent implements OnInit, OnDestroy {
      ...
      .subscribe(
        (res: string[]) => {
          ...
          const templates = this.templates.filter(
            (template) => template.name !== 'newTemplate'
          );
          if (templates.length) {
  ...
  getTemplateCategories(templateName: string): void {
    if (templateName !== 'newTemplate') {
      this.categoriesApiService
        .getTemplateCategories(templateName)
        .pipe(take(1))
        .subscribe(
    ...
    if (!this.dialogOpened) {
      ...
      ref.onClose.pipe(take(1)).subscribe(async (data) => {
        ...
        if (data) {
          ...
          ].value = data.newTemplateName.replace(/\s/g, '-');
          ...
          if (data.templateToExtend) {
            if (data.templateToExtend !== 'newTemplate') {
              await this.categoriesApiService
                .getTemplateCategoriesP(data.templateToExtend)
                .then((res: CategoryTemplateInterface) => {
                  this.renderFormTemplate(res);
                });
            } else {
              this.resetForm();
          ...
          this.form
            .get('templateName')
            .patchValue(data.newTemplateName.replace(/\s/g, '-'));
        }
      });
    }
  }
  ...
  renderFormTemplate(
    formData: CategoryTemplateInterface,
    templateName?: string
  ): void {
    ...
    this.form
      .get('templateName')
      .patchValue(templateName || formData.templateName);
    if (!templateName) {
    ...
    formData.incomes.forEach((incomeCategory, i) => {
      ...
      const selectedIcon = this.icons.find(
        (icon) => icon.name === incomeCategory.icon
      );
      if (selectedIcon) {
    ...
    formData.outcomes.forEach((outcomeCategory, i) => {
      ...
      const selectedIcon = this.icons.find(
        (icon) => icon.name === outcomeCategory.icon
      );
      if (selectedIcon) {
      ...
      (res) => {
        this.dashboardService.handleRequestCallbackMessage(
          ...
          this.transloco.translate(
            'messages.message.categoryTemplateSaved.title'
          ),
          this.transloco.translate(
            'messages.message.categoryTemplateSaved.message'
          )
        );
        this.dashboardService.clearMessages(2000);
      },
    ...
    await this.categoriesApiService.removeCategoryTemplate(templateId).then(
      () => {
        this.dashboardService.handleRequestCallbackMessage(
          'success',
          this.transloco.translate(
            'messages.message.categoryTemplateRemoved.title'
          ),
          this.transloco.translate(
            'messages.message.categoryTemplateRemoved.message'
          )
        );
        this.resetForm();
        this.getTemplates();
